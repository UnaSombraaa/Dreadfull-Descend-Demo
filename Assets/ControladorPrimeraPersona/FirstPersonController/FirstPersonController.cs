...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Crouch()
    {
        if (isCrouched)
        {
            // Levanta al jugador a su altura original
            transform.localScale = originalScale;
            walkSpeed /= speedReduction;  // Restaura la velocidad normal de caminar

            isCrouched = false;
        }
        else
        {
            // Agacha al jugador, reduciendo su altura
            transform.localScale = new Vector3(originalScale.x, crouchHeight, originalScale.z);
            walkSpeed *= speedReduction;  // Reduce la velocidad de caminar
